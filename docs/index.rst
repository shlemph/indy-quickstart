...
This goal of this project is to create a quick start to the language, vocabulary, and getting started with HyperLedger INDY and Self-Soverign Identity systems. 

## Getting Started

Before you bother to launch a docker container, generate keys, post transactions take a few minutes to learn the vocabulary and process.  

A great number of debugging efforts could have been thwarted with a tiny amount of upfront understanding. [note to self: remember your rules] 

[ insert your rules here for the humor value to others -> at a future time ]  

* no time spent reading the entire procedure before you start is wasted
* assume that everything has changed, packages have been upgraded, will be broken and create unrecoverable failures
* Your job is not just to fix it when it breaks, it is to understand why and make it more fault-tollerant. 
...
![alt text](https://raw.githubusercontent.com/hyperledger/indy-node/master/docs/relationship-diagram.png "Indy Relationship Diagram")
...
A DID is the address of a public key on a blockchain ledger (provable with a private key) 

Every Relationship between parties will be identified with a unique DID 

That DID Unique to that Relationship provides a unique, secure, lifetime, encrypted private channel with the connected parties which can be another person, organization or thing 
...
Each Reply to write requests has a number of common metadata fields. Most of these fields are actually metadata fields of a transaction in the Ledger (see [transactions](transactions.md)).
...
- `txn` (dict):
    ...
    - `metadata` (dict):
        ...
        - `from` (base58-encoded string):
             ...
             It may differ from `did` field for some of transaction (for example NYM), where `did` is a target identifier (for example, a newly created DID identifier).
...
- `auditPath` (array of base58-encoded hash strings):
    ...
    (this is a cryptographic proof to verify that the new transaction has been appended to the ledger)
...
So, if key rotation needs to be performed, the owner of the DID needs to send a NYM request with `dest` and `verkey` only. `role` and `alias` will stay the same.
...
So, if a Claim Def needs to be evolved (for example, a key needs to be rotated), then a new Claim Def needs to be created by a new Issuer DID (`identifier`).
...
- `force` (boolean; optional):
    ...
    Whether we should apply transaction (schedule Upgrade) without waiting for consensus of this transaction.
...
- `force` (boolean; optional):
    ...
    Whether we should apply transaction (for example, move pool to read-only state) without waiting for consensus of this transaction.
...
(https://github.com/shlemph/indy-quickstart/contributing) for details on our code of conduct, and the process for submitting pull requests to us.

## Versioning

We use [SemVer](http://semver.org/) for versioning. For the versions available, see the [tags on this repository](https://github.com/shlemph/indy-quickstart/tags). 

## Authors

* **George Lambert** - *Initial work* - [marchon gmail](https://github.com/shlemph/indy-quickstart)


See also the list of [contributors](https://github.com/shlemph/indy-quickstart/contributors) who participated in this project.
...
This work was prepared to communicate information learned as part of the intaglio blockchain project. You may or may not benefit from the content below, but you are encouraged to explore it, revise it, and send a pull request. All non-spam pull requests will be considered while document status is "WORKING DOCUMENT"
